...
class Point {
  constructor(z, x, y, active = false) {
    this.z = z;
    this.x = x;
    this.y = y;
  ...
  getNeighborPoints() {
    return [
      ...
      { z: this.z - 1, x: this.x - 1, y: this.y + 0 },
      { z: this.z - 1, x: this.x - 1, y: this.y + 1 },
      { z: this.z - 1, x: this.x + 0, y: this.y - 1 },
      { z: this.z - 1, x: this.x + 0, y: this.y + 0 },
      { z: this.z - 1, x: this.x + 0, y: this.y + 1 },
      { z: this.z - 1, x: this.x + 1, y: this.y - 1 },
      { z: this.z - 1, x: this.x + 1, y: this.y + 0 },
      { z: this.z - 1, x: this.x + 1, y: this.y + 1 },
      { z: this.z + 0, x: this.x - 1, y: this.y - 1 },
      { z: this.z + 0, x: this.x - 1, y: this.y + 0 },
      { z: this.z + 0, x: this.x - 1, y: this.y + 1 },
      { z: this.z + 0, x: this.x + 0, y: this.y - 1 },
      { z: this.z + 0, x: this.x + 0, y: this.y + 1 },
      { z: this.z + 0, x: this.x + 1, y: this.y - 1 },
      { z: this.z + 0, x: this.x + 1, y: this.y + 0 },
      { z: this.z + 0, x: this.x + 1, y: this.y + 1 },
...
class World {
  ...
  // Method func to return points at a specific zIndex
  getPointsAtZ(zIndex) {
    if (zIndex !== parseInt(zIndex, 10)) {
      // construction must be made of Points
      throw new Error(
        `getPointsAtZ must be an INT. Currently it's: ${getPointsAtZ}`
      );
    }
    let returnArr = [];
    for (let point of this.points) {
      if (point.z === zIndex) {
        returnArr.push(point);
      }
    }
    return returnArr;
  }
  ...
  for (let c = 0; c < cycles; c++) {
    ...
    for (let [hash, point] of _world.points.entries()) {
      ...
      point.neighborPoints.forEach((neighbor, n) => {
        let _nPoint = new Point(neighbor.z, neighbor.x, neighbor.y);
    ...
    for (let [hash, point] of _world.points.entries()) {
      ...
      point.neighborPoints.forEach((neighbor, n) => {
        if (_world.points.has(`z${neighbor.z}x${neighbor.x}y${neighbor.y}`)) {
          let _p = _world.points.get(
            `z${neighbor.z}x${neighbor.x}y${neighbor.y}`
          );
          if (_p.isActive === true) {
            activeNeighbors++;
          } else {
      ...
      let _newP = new Point(point.z, point.x, point.y);
      if (point.isActive) {
